...
class HTTPSSLPinning {
  ...
  customHttpClient({
    ...
    try {
      ...
      (isTestMode) {
        ...
        bytes
        = (await rootBundle.load('certificates/certificate.cer')).buffer.asUint8List();
      }
  ...
  static
  Future<http.Client>
  createLEClient({bool isTestMode = false}) async {
    IOClient client
    = IOClient(await customHttpClient(isTestMode: isTestMode));
    return client;
  }
}
